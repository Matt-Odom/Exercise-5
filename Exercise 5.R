...
ggplotly(Trips %>% 
           mutate(Hour = hour(sdate), Min = min(sdate)) %>% 
...
panama_map <- get_stamenmap(
    bbox = c(left = min(ironman$lon), bottom = min(ironman$lat), right = max(ironman$lon), top = max(ironman$lat)), 
    maptype = "terrain",
    zoom = 13)
...
covidcasesanim <- covid_over_10 %>% 
  ggplot()+
  geom_map(map = states_map, aes(map_id = state, fill = cases, group = date))+
  expand_limits(x = states_map$lon, y = states_map$lat)+
  theme_map()+
  scale_fill_viridis_b()+
  labs("Covid 19 cases per 10,000 Residents")+
  transition_states(date)


covidcasesfinal <- animate(covidcasesanim, nframes = 200, end_pause = 10)

anim_save("covidcasesanim.gif")
```

```{r}
knitr::include_graphics("covidcasesanim.gif")
```
...
Put the link to your app here: https://modom.shinyapps.io/COMP_STAT_112/
